...
class PositionEmbedding(nn.Module):
    def __init__(self, input_seq, d_model): #args
        super().__init__()
        self.position_embedding = nn.Parameter(torch.zeros(1, input_seq, d_model))

    def forward(self, x):
        x = x + self.position_embedding #[ :args.seq_len]
        return x
...
class HighLevelTransformer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.position_embedding = PositionEmbedding(input_seq=(self.args.seq_len_hlt + 1), d_model=self.args.d_model)
