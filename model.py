...
class PositionEmbedding(nn.Module):
    def __init__(self, input_seq, d_model, args): #args
        super().__init__()
        self.position_embedding = nn.Parameter(torch.zeros(1, input_seq, d_model))
        self.args = args

    def forward(self, x):
        x = x + self.position_embedding[:self.args.seq_len_hlt] #[ :args.seq_len_hlt]
        return x
...
class HighLevelTransformer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.position_embedding = PositionEmbedding(input_seq=(self.args.seq_len_hlt + 1), d_model=self.args.d_model, args=self.args)
        # self.position_embedding = PositionEmbedding(input_seq=(self.args.num_token + 1), d_model=self.args.d_model)
    ...
    def forward(self, tuples):
        ...
        # x = x[:, :self.args.num_token, :]
        x = torch.cat((self.class_embedding.expand(b, -1, -1), x), dim=1)
        # x = self.position_embedding(x)
        x = x + nn.Parameter(torch.zeros(1, x.shape[1], x.shape[2])).to(self.args.device)   # positional embedding
        # if np.shape(x)[0] == 64:
            # To make shorter interval
        # c = []
        # sums = 0
        # for j in range(1, 65):
        #     self.args.seq_len_hlt = j
        #     x =x[:, :self.args.seq_len_hlt, :]
...
if __name__ == '__main__':
    ...
    dummy = torch.zeros((2, args.window_len, args.eeg_ch)).to(args.device)
    ff = model(dummy)
    pass
